...
def main(args):
	...
	logger.debug(f"Loading Pretrained Models")
	for i in range(len(args.arch)):
	
		per_model, msg, feat_dim = get_model(args.arch[i], skip_pool=args.skip_pool, \
			pretrain_path = None if len(args.pretrained)==0 else args.pretrained[i], \
			img_dim=datamsg['img_dim'])
		#fix1st_pretrain_path = args.fix1st_pretrained ) #e.g. 'regnet_y_32gf'
		logger.info("Load pretrained model with msg: {}".format(msg))
		assert len(msg.missing_keys) == 0, "Weights Loaded Incorrectly"			
	...
	elif args.scheduler_type == "schedulefree_adamw":
		scheduler = None
	logger.info(f"lr scheduler {scheduler}")
	...
	for epoch in range(start_epoch, args.epochs):
		...
		if (epoch+1) % args.eval_freq == 0: 
			loss, top1, top5 = validate_network(val_loader, model,  classifier, args, optimizer)
			...
			for key in additional_loaders:
				...
				ad_loss, ad_top1, ad_top5 = validate_network(loader, model,  classifier,args, optimizer)
			...
			if args.rank == 0:
				logger.info(
					...
					"Loss {loss:.4f}\t"
					"Acc@1 {top1:.3f}\t"
					"Best Acc@1 so far {acc:.2f}".format(loss=loss, top1=top1, acc=best_acc))
...
def validate_network(val_loader, model, classifier, args, optimizer, indices=None):
	batch_time = AverageMeter()
	losses = AverageMeter()
	top1 = AverageMeter()
	top5 = AverageMeter()
...
def custom_params():
	...
	parser.add_argument('--start_over', default=False, type=bool_flag,help='do not load saved checkpoints')
	
 
	#########################
	#### optim parameters ###
	#########################
	parser.add_argument("--optimizer", default='sgd', type=str, help='sgd, adam,schedulefree_adamw')
	...
	parser.add_argument("--momentum", default=0.9, type=float, help="momentum in SGD, beta1 in adam, beta in sf")
